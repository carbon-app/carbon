...
async function updateGist(uid, { filename, code, config }) {
  if (!uid || !filename) {
    return
  }
  try {
    const files = {}
    if (config) {
      files[CARBON_STORAGE_KEY] = JSON.stringify(config)
    }

    files[filename] = code

    return await githubClient.patch(`/gists/${uid}`, { files }).then(console.log)
  } catch (error) {
