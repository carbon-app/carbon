...
export const getRouteState = location => {
  const { query, pathname } = url.parse(location, true)
  const state = getQueryStringState(query)
  const path = escapeHtml(pathname.split('/').pop())
  const gistUri = path.length >= 19 && path.indexOf('.') === -1 && path
